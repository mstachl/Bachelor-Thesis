...
\section{Niedring-Rang-Approximation einer Matrix}
	...
	\subsection{CUR-Zerlegung}	
		...
		\begin{minipage}{16cm}
		\vspace{5mm}
			\textbf{CUR-Zerlegung mit 2-Norm-Rating} \newline
			\newline
			\begin{tabular}{ll}
				\textbf{Input:} & Matrix $V\in\mathds{R}^{m\times n}$, Reihenanzahl $r\leq m$, Zeilenanzahl $c\leq n$,\\
				 &  Zielrang $k\leq\min\{r,c\}$, $\{p_i\}_{i=1}^m$ und $\{q_j\}_{j=1}^n$ \\
				(i) & für $t=1\dots c$\\
				 &  die Matrix $C$ \\
		...
		\newline
		\caption{Der Algorithmus zur Bestimmung der CUR-Zerlegung mithilfe des 2-Norm-Rankings}
		\end{table}
		\textbf{Nachteile:}
		\begin{itemize}
			\item Durch die zufällige Auswahl der Spalten und Zeilen kann man nur etwas über den zu erwartenden 
			Fehler aussagen.
			\item Der resultierende Fehler zwischen Approximation und Zielmatrix ist additiv und abhängig von $||V||_F$
		\end{itemize}
